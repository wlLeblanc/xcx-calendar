...
Page({
  data: {
    maxMonth: 3, //最多渲染月数
    dateList: [],
    startTime: ['8:00', '8:30', '9:00', '9:30', '10:30', '11:00', '11:30', '12:00', '12:30', '13:00'],
    tempYearPos:'',
    systemInfo: {},
  ...
  selectDataMarkLine: function () {
    ...
    let curreInid = checkInDate.substr(0, 4) + "-" + (checkInDate.substr(5, 2) < 10 ? checkInDate.substr(6, 1) : checkInDate.substr(5, 2)); //选择入住的年-月id
    let curreOutid = checkOutDate.substr(0, 4) + "-" + (checkOutDate.substr(5, 2) < 10 ? checkOutDate.substr(6, 1) : checkOutDate.substr(5, 2)); //选择离店的年-月id
    ...
    if (curreInid == curreOutid) { //入住与离店是当月的情况
      ...
      for (let j = 0; j < dateList.length; j++) {
        if (dateList[j].month == monthIn) { //入住的开始月份
          ...
          if (Moment(dateList[j].id).differ(curreOutid) < 0 && Moment(dateList[j].id).differ(curreInid) > 0) { //离店中间的月份
            let days = dateList[j].days;
            for (let k = 0; k < days.length; k++) {
              days[k].class = days[k].class + ' bgitem';
            }
